...
//} ScaleParams;
//
//static int ProxyMain(GDALDataset* dataset, int argc, char ** argv )
//
//{
//    GDALDatasetH hOutDS;
//    int			i;
...
//
//    if( !bCopySubDatasets 
//        && CSLCount(dataset->GetMetadata("SUBDATASETS" )) > 0 
//        && dataset->GetRasterCount() == 0)
//	{
//        fprintf( stderr,
...
//    }
//
//	if( CSLCount(dataset->GetMetadata("SUBDATASETS")) > 0 
//        && bCopySubDatasets )
//    {
...
//			nRet = ProxyMain(dataset, argc, papszDupArgv );
//            if (nRet != 0)
//                break;
//        }
...
//    if( nBandCount == 0 )
//    {
//		nBandCount = dataset->GetRasterCount();
//        if( nBandCount == 0 )
//        {
...
//        for( i = 0; i < nBandCount; i++ )
//        {
//			if( ABS(panBandList[i]) > dataset->GetRasterCount())
//            {
//                fprintf( stderr, 
//                         "Band %d requested, but only bands 1 to %d available.\n",
//						 ABS(panBandList[i]), dataset->GetRasterCount() );
//                GDALDestroyDriverManager();
//                exit( 2 );
//            }
//        }
//
//		if( nBandCount != dataset->GetRasterCount())
//            bDefBands = FALSE;
//    }
//
...
//		|| anSrcWin[0] + anSrcWin[2] > dataset->GetRasterXSize()
//		|| anSrcWin[1] + anSrcWin[3] > dataset->GetRasterYSize())
//    {
//        int bCompletelyOutside = anSrcWin[0] + anSrcWin[2] <= 0 ||
//                                    anSrcWin[1] + anSrcWin[3] <= 0 ||
//									anSrcWin[0] >= dataset->GetRasterXSize() ||
//									anSrcWin[1] >= dataset->GetRasterYSize();
...
//		   && anSrcWin[2] == dataset->GetRasterXSize()
//		   && anSrcWin[3] == dataset->GetRasterYSize()
...
//    {
//        
//        hOutDS = GDALCreateCopy( hDriver, pszDest, dataset, 
...
//    }
//
//	else if( dataset->GetGeoTransform(adfGeoTransform) == CE_None 
//        && nGCPCount == 0 )
//    {
...
//			pszGCPProjection = dataset->GetGCPProjection();
//        if( pszGCPProjection == NULL )
//            pszGCPProjection = "";
//
...
//    }
//
//	else if( dataset->GetGCPCount() > 0 )
//    {
//        GDAL_GCP *pasGCPs;
//		int       nGCPs = dataset->GetGCPCount();
//
//		pasGCPs = GDALDuplicateGCPs( nGCPs, dataset->GetGCPs() );
//
//        for( i = 0; i < nGCPs; i++ )
//        {
...
///* -------------------------------------------------------------------- */
//    char** papszMetadata = CSLDuplicate((dataset)->GetMetadata());
...
//
//	const char* pszInterleave = dataset->GetMetadataItem("INTERLEAVE", "IMAGE_STRUCTURE");
//    if (pszInterleave)
...
//        char **papszMD;
//
//        papszMD = (dataset)->GetMetadata("RPC");
//        if( papszMD != NULL )
//            poVDS->SetMetadata( papszMD, "RPC" );
//
//        papszMD = (dataset)->GetMetadata("GEOLOCATION");
//        if( papszMD != NULL )
...
//        poSrcBand = (dataset)->GetRasterBand(ABS(panBandList[0]));
//        if (panBandList[0] < 0)
...
//
//        poSrcBand = (dataset)->GetRasterBand(ABS(nSrcBand));
//
///* -------------------------------------------------------------------- */
...
//			(GDALGetMaskFlags(dataset->GetRasterBand(1)) & GMF_PER_DATASET) == 0 &&
...
//    {
//        GDALRasterBand *poSrcBand =
//			(GDALRasterBand*)dataset->GetRasterBand(ABS(nMaskBand));
...
//		GDALGetMaskFlags(dataset->GetRasterBand(1)) == GMF_PER_DATASET)
//    {
//        if (poVDS->CreateMaskBand(GMF_PER_DATASET) == CE_None)
//        {
//            VRTSourcedRasterBand* hMaskVRTBand = (VRTSourcedRasterBand*)
//                GDALGetMaskBand(GDALGetRasterBand((GDALDatasetH)poVDS, 1));
//			hMaskVRTBand->AddMaskBandSource((GDALRasterBand*)dataset->GetRasterBand(1),
//                                        anSrcWin[0], anSrcWin[1],
//                                        anSrcWin[2], anSrcWin[3],
//                                        anDstWin[0], anDstWin[1],
//                                        anDstWin[2], anDstWin[3] );
...
//		(char*)inImgPath.c_str() ,(char*)outImgPath.c_str(),
//		"-srcwin", (char*)widthPoint.c_str(), (char*)heightPoint.c_str(),
//		(char*)widthPixelLen.c_str(), (char*)heightPixelLen.c_str()};
//
//	//15为argv中字符串的个数
//    return ProxyMain(srcImgDataset,14, argv);
//}
//
//
